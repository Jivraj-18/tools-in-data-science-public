# Hands-on — Week 08 — MLOps, Fine-Tuning & Model Publishing

Train, fine-tune, package, publish, and monitor models: data/versioning, MLflow, Vertex AI, HuggingFace, quantization, and responsible model release.

## Assignments

- [CAPSTONE: Production Fine-Tuned Model + Full MLOps](./capstone-production-finetuned-model-mlops)
- [Lab: Full GCP pipeline (GCS → BigQuery ML → Vertex AI → Cloud Run)](./full-gcp-pipeline-bqml-vertex-cloud-run)
